...
Tests.test("Sim ea3-123: HoneyHive solo < AcornSniper solo (DOT < direct damage)", function()
    ...
    -- ea3-239b widened 5 → 7 to absorb ea3-234's HoneyHive range
    -- 20 → 30 buff (sim DOT term grew ~50% via exposureSecs).
    -- ea3-244 (2026-05-04): patch-overlap saturation factor landed in
    -- dotDamagePerShot. Honey's effective dotPerShot now scales by
    -- ~0.57 when patches overlap (spacing 8 < diameter 14). Sim
    -- prediction should drop from ~15.79 toward ~12-13, and the
    -- validator-flagged HoneyHive signed +6.66 over-prediction
    -- should close substantially. Margin held at 7 for one more
    -- sweep cycle so the test remains a guard while the fix
    -- proves out; tighten back to 5 once validator confirms
    -- HoneyHive signed |Δ| < 1.5 wave.

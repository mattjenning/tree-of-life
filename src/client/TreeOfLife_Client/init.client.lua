...
-- ============================================================
-- INFINITE STUDIO auto-place: REMOVED 2026-05-03 (ea3-238).
-- Per Matthew "SELECT and GO should use ArenaSweepRunner with
-- AutoPlaceStrategy" — placement moved server-side to
-- Infinite.placeAllForLoadoutServer (calls
-- AutoPlaceStrategy.findOptimalCell per tower against live
-- placedAllies, same algorithm ArenaSweepRunner uses for failure
-- curves). The 200+ lines of client placeInfinitePattern + the
-- InfiniteAutoPlace OnClientEvent dispatch are gone — server
-- places towers directly via PlaceTower internal API, no client
-- roundtrip. Story-mode placeAllTowers (above) is the only
-- remaining client-side placement helper, and it's used only by
-- the DevAutoPlace flag on Map 1.
-- ============================================================
